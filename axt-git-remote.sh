...
# HELP: Choose remote branch from list and use/create matching or user-defined local branch
...
command -v peco  >/dev/null 2>&1 || { echo >&2 "AXT ERROR: axt git remote requires helper 'peco'!"; exit 1; }
...
if (( $# <= 1 )); then
	...
	echo >&2 "AXT ERROR: too many parameters for subcommand 'git remote'"
	exit 1
fi
